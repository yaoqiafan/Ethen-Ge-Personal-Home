<template>
  <div class="card ai-card">
    ...
    <div class="ai-subtitle">本地 API Key · OpenAI 兼容协议</div>
    ...
    <div v-else class="empty-state">暂无对话记录</div>
...
import { getConfig, loadConversations } from '@/services/aiService'
import type { Conversation } from '@/services/aiService'

const conversations = ref<Conversation[]>([])
const apiConfigured = computed(() => !!getConfig().apiKey)
...
/* ── 最近对话 ─────────────────────────────────────────────────── */
.recent-list { display: flex; flex-direction: column; gap: 2px; flex: 1; }
